...
- 「サイドボタンが分からない」: 「ブラウザで押すと前のページに戻るボタンです。多くのマウスで左わき・親指の当たる場所にあります」と案内する。サイドボタンが無くても、Ctrlキーの単押しで一覧を開けることも必ず添える
...
- 「一覧が出ない」: Ctrlは単押し（押しっぱなしにしない）。0.35秒以上の長押しは反応しない仕様
...
export const CANNED_USAGE_OVERVIEW = `送信サジェストの主な使い方です。

まずこれ↓
・Ctrlキーをポンと1回押す（押しっぱなしにしない）
　→ コピペ履歴と定型文の一覧が開きます
　→ 数字キー(1〜9,0)でその行を貼り付け
　→ マウスのサイドボタンでも開きます
