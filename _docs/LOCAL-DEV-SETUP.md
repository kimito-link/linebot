...
## ★もう1つの地雷: next dev 中に next build しない

同じ `.next` を共有しているので、dev サーバーを動かしたまま build すると
チャンクが差し替わり、dev 側が `Cannot find module './4128.js'`
（MODULE_NOT_FOUND）で500を返すようになる。**コードの問題ではない。**
→ dev を止めてから build する。踏んだら dev を再起動すれば直る。
